 ...
 *   - unsigned 8-bit integer array
 */
export function sha256(input: uint8mixed, toUint8: true): number[];
export function sha256(input: uint8mixed, toUint8?: false): string;
export function sha256(input, toUint8 = false): uint8mixed {
  let bytes = input;
  ...
  bytes.push(0x80);
  // Convert to words
  const words = uint8ArrayToUint32Array(bytes);
  ...
  // Return the result
  if (toUint8) {
    return uint32ArrayToUint8Array(hash);
  }
  return uint32ArrayToHex(hash);
}
