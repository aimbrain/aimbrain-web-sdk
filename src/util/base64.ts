...
export function encode(input: uint8mixed): string {
...
export function decode(input: string, toUint8: true): number[];
export function decode(input: string, toUint8?: false): string;
export function decode(input, toUint8 = false): uint8mixed {
    const output: number[] = [];
    // Clean up input
